...
class RAGWorkflow:
    ...
    def _build_graph(self):
        ...
        # query_rewriter runs first, then fan-out to fetch_documents and generate_summary
        # llm_call waits for both branches to complete (fan-in)
        graph.add_edge(START, "query_rewriter")
        graph.add_edge("query_rewriter", "fetch_documents")
        graph.add_edge("query_rewriter", "generate_summary_last_5_messages")
